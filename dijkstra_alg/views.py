        ...
        for key, val in possible_next_n:
            try:
                if shortest_dist[key] > val + shortest_dist[closest]:
                    ...
                    print("NODE ADDITION", prev_nodes[key])
            except KeyError:
                break
...
#The main principle is that the data are divided in half as many times as possible and
#with regards to the x and y axis. Moreover, it creates the branches of the tree
...
#dist() function takes two node arguments and returns their squared distance.
...
#tree that the desired point exists and visits that reducing the number of items it has to go through.
#That is because each time, the number of items that must be searched is reduced by a factor of 1/2.
...
#the depth is set and counts the "distance" from the root node.
def closest(current, node, height=0):
    split_point = height % dim
    if current is None:
        return None
    # next = None
    ...
    print(type(current['node'][split_point]))
    start_pos = current['node'][split_point]
    ...
    #final is of type node and is equal to the function
    final = compare(node, closest(next, node, height + 1), current['node'])
